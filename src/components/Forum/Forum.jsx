...
import { MessageCircle, Gamepad2, Tv, BookOpen, Coffee, Plus, Search, TrendingUp, Clock, Heart, Image, X, Eye, Bold, Italic, Upload, Link as LinkIcon, List, Quote, AlertCircle, Loader2, Flame, Hash, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  return (
    <div className="forum-page">
      ...
      <div className="forum-board-cards">
        <div
          className={`forum-board-card ${!activeBoard ? 'active' : ''}`}
          onClick={() => setActiveBoard(null)}
        >
          <div className="board-card-icon all">
            <MessageCircle size={20} />
          </div>
          <div className="board-card-info">
            <span className="board-card-name">全部</span>
            <span className="board-card-desc">{posts.length}篇帖子</span>
          </div>
        </div>
        {BOARDS.map(board => {
          ...
          return (
            <div
              key={board.key}
              className={`forum-board-card ${activeBoard === board.key ? 'active' : ''}`}
              style={{ '--board-color': board.color }}
              onClick={() => setActiveBoard(activeBoard === board.key ? null : board.key)}
            >
              <div className="board-card-icon">
                <Icon size={20} />
              </div>
              <div className="board-card-info">
                <span className="board-card-name">{board.label}</span>
      ...
      <div className="forum-hot-section">
        <button className="hot-section-toggle" onClick={() => setShowHotSection(!showHotSection)}>
          <Flame size={14} /> 热门内容
          {showHotSection ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
        </button>
        {showHotSection && (
          <div className="hot-section-content">
            <div className="hot-section-grid">
              {/* 热门帖子 */}
              <div className="hot-posts-panel">
                <h4 className="hot-panel-title"><Flame size={13} /> 热门帖子</h4>
                {hotPosts.map((post, idx) => (
                  <Link to={`/forum/post/${post.id}`} key={post.id} className="hot-post-item">
                    <span className="hot-post-rank">{idx + 1}</span>
                    <div className="hot-post-info">
                      <span className="hot-post-title">{post.title}</span>
                      <span className="hot-post-meta"><Eye size={10} /> {post.views || 0} · <Heart size={10} /> {post.likes || 0}</span>
                    </div>
                  </Link>
                ))}
              </div>
              {/* 热门标签 */}
              {hotTags.length > 0 && (
                <div className="hot-tags-panel">
                  <h4 className="hot-panel-title"><Hash size={13} /> 热门标签</h4>
                  <div className="hot-tag-cloud">
                    {hotTags.map(tag => (
                      <button key={tag} className="hot-tag-pill" onClick={() => setSearchQuery(tag)}>{tag}</button>
                    ))}
                  </div>
                </div>
              )}
            ...
            return (
              <Link to={`/forum/post/${post.id}`} key={post.id} className="forum-post-card">
                <div className="post-card-top">
                  <UserAvatar userId={post.author_id} src={author?.avatar} alt={author?.name} size={36} className="post-user-avatar" />
                  ...
                  <div className="post-card-stats">
                    <span className="post-stat"><Heart size={13} /> {post.likes || 0}</span>
                    <span className="post-stat"><MessageCircle size={13} /> {post.replies_count || 0}</span>
                    <span className="post-stat"><Eye size={13} /> {post.views || 0}</span>
                  </div>
                </div>
                ...
                {postImages.length > 0 && (
                  <div className="post-card-images">
                    {postImages.slice(0, 3).map((img, i) => (
                      <div key={i} className="post-card-img-wrap">
                        <img src={typeof img === 'string' ? img : img.preview} alt="" className="post-card-img" loading="lazy" />
                        {i === 2 && postImages.length > 3 && (
                          <span className="post-card-img-more">+{postImages.length - 3}</span>
                        )}
                      </div>
                    ))}
                  </div>
